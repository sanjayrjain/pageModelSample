...
public class BaseFactory extends CoreBase {
		...
		try {
			if (browser.toLowerCase().contains("firefox")) {
				...
				if (headerEnable) {
					...
					proxy.start();
					proxy.addHeader("FKUserAgent", "FK/testenvironment/1.0");
	...
	static ExtentReports createExtentReportInstance() {
		...
		htmlReporter.config().setDocumentTitle("FK - Automation Execution");
		htmlReporter.config().setReportName("Desktop Sanity Test Suite - Results");
		...
		extent.setSystemInfo("Author", "FK-QE Automation Team");
